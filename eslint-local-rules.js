...
function isZustandFunction(variableName, calleeName) {
  return (
    useZustandStores.includes(calleeName) &&
    (zustandFunctions.includes(variableName) ||
      zustandFunctions.some(
        fn =>
          variableName.startsWith(fn) &&
          variableName.length > fn.length &&
          variableName[fn.length] === variableName[fn.length].toUpperCase(),
      ))
  );
}
...
function isReactReanimatedFunction(variableName, calleeName) {
  return (
    ReactReanimatedFunctions.includes(calleeName) ||
    Object.entries(ReactReanimatedHooks).some(
      ([hook, variable]) => variableName === variable && calleeName === hook,
    )
  );
}

var globalUnnecessaryDependencies = new Set();
function isUnnecessaryDependency(variableName, resolved) {
  if (globalUnnecessaryDependencies.has(variableName)) {
    return true;
  }

  if (!resolved || !resolved.defs || resolved.defs.length === 0) {
    return false;
  }
  ...
  if (
    isZustandFunction(variableName, calleeName) ||
    isReactReanimatedFunction(variableName, calleeName)
  ) {
    globalUnnecessaryDependencies.add(variableName);
    return true;
  }
  return false;
}
...
module.exports = {
  'exhaustive-deps': {
    ...
    create(context) {
      globalUnnecessaryDependencies = new Set();
          ...
          for (const reference of currentScope.references) {
            ...
            if (!dependencies.has(dependency)) {
              ...
              const isStable =
                ...
                isUnnecessaryDependency(dependency, resolved);
...
function collectRecommendations({
  ...
  declaredDependencies.forEach(({key}) => {
    if (globalUnnecessaryDependencies.has(key)) {
